...
**`lib/sheet-snapshot.json`** — safety net: gid map ชุดล่าสุดที่ยืนยันแล้ว commit ไว้ในรีโป
ถ้า `discoverSheets()` พัง (Google เปลี่ยนรูปแบบ HTML / โดน rate limit) → ใช้ snapshot แทน **เว็บไม่ล่ม** พร้อมขึ้น banner เตือนว่ากำลังใช้รายชื่อแท็บสำรอง
มีสคริปต์ `bun run sync:sheets` เขียนทับไฟล์นี้จากผลค้นหาล่าสุด (รันเมื่อยืนยันว่าข้อมูลถูก)

**ผลลัพธ์: เทอมหน้าเปลี่ยน Sheet = เปลี่ยนแค่ `EXAM_SHEET_ID` ตัวเดียว** แท็บ/gid/วิชา/ห้อง ระบบไล่เก็บเองทั้งหมด
...
- รหัสวิชา: `/^\s*([A-Z]{2}\d{6})\s*:?\s*(.*)$/` — **ถ้าไม่ match ให้ fallback เป็นชื่อดิบ ห้าม throw** (มี 1 รายการรูปแบบเพี้ยน: `614201 TOXIC SUBSTANCES IN DAI…`)
...
- **การเรียงลำดับเวลาต้องคำนวณจากข้อมูล ห้ามพึ่งลำดับแท็บ**: แปลง `24-ส.ค.-69` ด้วย map เดือนไทย (`ม.ค.`…`ธ.ค.`) + ปี พ.ศ. 2 หลัก (69 → 2569 → ค.ศ. 2026) แล้วรวมกับเวลาเริ่ม (`08.30` / `13.00`) เป็น sort key
...
**`app/api/revalidate/route.ts`** — `POST` + secret header → `revalidateTag("exam-data")` สำหรับสั่ง refresh ทันทีเมื่อ Sheet แก้
...
- `app/robots.ts`: `Disallow: /student/`
...
0. **การค้นหาแท็บอัตโนมัติ** — `discoverSheets()` ต้องคืน **11 แท็บ** ตรงตามที่ตรวจไว้แล้ว
   ...
   แล้วลองตัดเน็ต/ใส่ id มั่ว → ต้องตกไปใช้ snapshot พร้อมขึ้น banner เตือน ไม่ใช่ 500
